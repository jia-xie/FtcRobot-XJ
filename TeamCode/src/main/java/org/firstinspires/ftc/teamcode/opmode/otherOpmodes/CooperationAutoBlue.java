...
@Config
@Disabled
public class CooperationAutoBlue extends LinearOpMode {
    ...
    Position highGoalLaunchPositionI = new Position(48,69,24);
    Position highGoalLaunchPositionII = new Position(34.5,69,10);
    //PID controller
    ...
    public void runOpMode() throws InterruptedException {
        ...
        switch(ringCase){
            case "Single":
                targetZonePosition.setPosition(2 * 23 + 9,5 * 23 - 9,-90);
                break;
            case "Quad":
                targetZonePosition.setPosition(23 + 9,6 * 23 - 7.5,-90);
                break;
        }

//        motorControl(ejectPowerShotSpeed,ejectPowerShotPower);
//        positionAccurateMove(1200,new Position(48,69,firstAngle),5000,1000);
//
//        sleep(200);
//        singlePush();
...
//        servoUpDown.setPosition(downIndex);
        ...
        positionAccurateMove(1000, new Position(48,69,24),4500,2000);
        multiplePush(4,200);

        servoUpDown.setPosition(downIndex);
        positionAccurateMove(2000,new Position(3 * 23 - 7, 2 * 23 + 11.5, 10), 3000,1000);
...
//        while(opModeIsActive() && !isPathCollected){
//            while (!pipeline.finishFlag);
//            for (Ring ring : pipeline.getRing()) {
//
//                Position target = ring.getPosition();
//
//                ringCollectPathPositions.add(new Position(
//                        current.x + target.x, current.y + target.y));
//            }
//            isPathCollected = false;
//        }
        ...
        followPath(1,ringCollectPathPositions,10000);
        ...
        //Parking
        if(ringCase == "null"){
            positionAccurateMove(2000,new Position(23 * 2 + 11.5,80.5,0),3000,2000);
        }else {
            ArrayList<Position> parkingPathPositions = new ArrayList<>();
            parkingPathPositions.add(odometry.getCurrentPosition());
            switch (ringCase) {
                case "Single":
                    parkingPathPositions.add(new Position(57.5, 5 * 23 - 11));
                    break;
                case "Quad":
                    parkingPathPositions.add(new Position(57.5, 6 * 23 - 11));
                    break;
            }
            parkingPathPositions.add(new Position(57.5,80.5));
